...
const AppContent = ({ cart, setCart, emailForOtp, setEmailForOtp }) => {
  ...
  return (
    <div>
      {!isAdminRoute && <Header cart={cart} />} {/* Header chỉ xuất hiện nếu không phải admin */}
      <Routes>
        ...
        <Route path="/product/:productId" element={<ProductDetail addToCart={(product) => setCart((prev) => [...prev, product])} />} />
        ...
        <Route path="/admin/editproducts/:productId" element={<ProductEditPage />} />
        <Route path="/admin/request-otp" element={<RequestOtpPage setEmailForOtp={setEmailForOtp} />} />
      ...
      {!isAdminRoute && <Footer />} {/* Footer chỉ xuất hiện nếu không phải admin */}
    </div>
  );
};
